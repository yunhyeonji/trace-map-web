...
const Header = () => {
  ...
  const isCreatePage = pathname === '/travels/list/create';
  const isDetailPage = pathname?.match(/\/travels\/list\/\d+$/);
  const showBackButton = isCreatePage || isDetailPage;
    ...
    <header className="bg-background/80 sticky top-0 z-30 border-b shadow backdrop-blur">
        ...
        {showBackButton ? (
          ...
          <>
            ...
            <span className="text-sm font-semibold">
              {isCreatePage ? '여행 기록 작성' : '여행 상세'}
            </span>
          </>
        ) : (
          // 목록 페이지 - 로고
          <Link href="/travels/dashboard" className="flex min-w-0 items-center gap-2">
      ...
      <div className="container hidden h-(--header-height) items-center gap-4 md:flex">
        {showBackButton ? (
          ...
          // 목록 페이지 - 로고 + 검색
          <>
            <Link href="/travels/dashboard" className="flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          {!isCreatePage && (
            <Link
              ...
              className="hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50 rounded-full p-2 transition-colors"
            >
